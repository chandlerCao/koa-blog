...
class articleModel {
    ...
    async getArticleList(ip, type, skip, len) {
        const sql = `select atc.aid, atc.title, atc.preface , atc.cover, atc.tag_id, atc.type_id, atc.date, atc.read_count, tag.tag_name, count(al.aid) as like_count, (select count(*) from art_like where uip = ? and aid = atc.aid) as is_like
        from article as atc
